...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 shrink-0 h-full border-r border-zinc-800/80 bg-zinc-950/70 backdrop-blur-2xl p-4 overflow-hidden select-none z-20">
      {/* Brand Logo */}
      <Link href="/" className="flex items-center gap-3 px-3 py-4 mb-2">
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-500 flex items-center justify-center shadow-lg shadow-indigo-500/25">
          <Disc3 size={24} className="text-white animate-[spin_10s_linear_infinite]" />
        </div>
        <div>
          ...
          <p className="text-[11px] text-zinc-400">Stream & Offline Audio</p>
        </div>
      </Link>
      ...
      {/* Offline Mode Switch */}
      <div className="my-3 px-3 py-2.5 rounded-xl bg-zinc-900/60 border border-zinc-800/80 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-zinc-300">Modalità Offline</span>
        </div>
        <input
          type="checkbox"
      ...
      <div className="flex-1 overflow-y-auto mt-2 pr-1 border-t border-zinc-800/60 pt-3">
            ...
            <Link
              ...
              className={`block px-3 py-1.5 rounded-lg text-xs truncate transition-colors ${
                pathname === `/playlists/${pl.id}`
                  ? "text-indigo-400 font-semibold bg-zinc-900"
